...
const Courses = (props: CoursesProps) => {
  ...
  const renderItem = ({ item }: { item: ICourse }) => {
    return (
      <TouchableOpacity className="bg-white rounded-2xl p-3 border-2 border-[#00000029] w-fit h-fit ">
        <FastImage
          source={{
            uri: item?.thumb,
          }}
        ...
        <Text className="font-outfit text-base text-violet">{item?.type}</Text>
      </TouchableOpacity>
    )
  }
  return (
    <View className="gap-2 w-fit h-fit">
      <Text className="font-outfit text-2xl text-black font-bold">{title}</Text>
      <FlatList
        data={courses}
